...
def resolve_import_conflict(content_a, content_b, filepath):
    ...
    for i, line in enumerate(lines_a):
        if line.strip().startswith("from ") or line.strip().startswith("import "):
            ...
            if m:
                ...
                items = [x.strip() for x in line.strip().replace("import ", "").split(',')]
    ...
    if first_import_idx is not None:
        ...
        new_lines = lines_a[:first_import_idx] + import_lines + lines_a[last_import_idx + 1:]
    ...
    if first_import_idx is not None:
        new_lines = lines_b[:first_import_idx] + import_lines + lines_b[last_import_idx + 1:]
        return "".join(new_lines)
...
def resolve_file_merge(file_path, content_a, content_b, scenario_a, scenario_b, main_dir):
    ...
    if ext in (".py",):
        ...
        has_imports_a = any(line.strip().startswith(("from ", "import ")) for line in content_a.splitlines())
        has_imports_b = any(line.strip().startswith(("from ", "import ")) for line in content_b.splitlines())
        ...
        if has_imports_a or has_imports_b:
            ...
            if merged != content_a and merged != content_b:
                log_event(log_path, "import_merge", file=file_path, 
    ...
    for func_name, line_idx in test_funcs_b.items():
        if func_name not in test_funcs_a:
            ...
            merged_lines = merged_lines[:insert_idx] + func_lines + merged_lines[insert_idx:]
            offset += len(func_lines)
            log_event(log_path, "test_function_added", file=file_path,
                     function=func_name, scenario=scenario_b)
    ...
    for i, line in enumerate(lines_b):
        if f"# BDD: {scenario_b}" in line:
            ...
            if not found:
                ...
                for j, aline in enumerate(lines_a):
                    if "def test_" in aline and scenario_b.lower().replace(" ", "_") in aline.lower():
                        marker_line = f"# BDD: {scenario_b}\n"
                        merged_lines.insert(j, marker_line)
                        log_event(log_path, "marker_added", file=file_path,
                                 scenario=scenario_b, line=j)
                        break
    ...
    for i, result in enumerate(scenario_results):
        ...
        if rc != 0:
            ...
            for conflict_file in conflict_files:
                ...
                log_event(log_path, "conflict_resolved", file=conflict_file, 
                         scenario=scenario)
            ...
            if rc != 0:
                log_event(log_path, "merge_commit_failed", scenario=scenario, error=stderr)
