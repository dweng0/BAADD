...
def test_dashboard_file_exists_at_scripts_dashboard_py():
    path = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'scripts', 'dashboard.py'))
...
def test_each_agent_rendered_as_panel():
    ...
    state = AgentState(wt_path='/tmp/test', scenario_name='My Scenario', start_ts=time.time())
...
def test_slug_to_name_converts_path():
    ...
    assert slug_to_name("/tmp/baadd-wt-enable-verbose-mode-5678") == "enable verbose mode"
...
def test_read_wt_state_current_iter():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        _write_jsonl(os.path.join(tmpdir, "agent_events_se_1_test.jsonl"), [
...
def test_read_wt_state_max_iter():
    from dashboard import read_wt_state
    with tempfile.TemporaryDirectory() as tmpdir:
        _write_jsonl(os.path.join(tmpdir, "agent_events_se_1.jsonl"), [
            {"event": "iteration_start", "iteration": 1, "max_iterations": 125},
        ])
        state = read_wt_state(tmpdir)
...
def test_read_wt_state_active_phase():
    from dashboard import read_wt_state
    with tempfile.TemporaryDirectory() as tmpdir:
        _write_jsonl(os.path.join(tmpdir, "agent_events_pm_plan_1.jsonl"), [
...
def test_read_wt_state_skips_malformed_json():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(path, 'w') as f:
            f.write('{"event": "iteration_start", "iteration": 1, "max_iterations": 100}\n')
            f.write('not valid json\n')
            f.write('{"event": "iteration_start", "iteration": 5, "max_iterations": 100}\n')
        state = read_wt_state(tmpdir)
...
def test_read_wt_state_picks_highest_mtime():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _write_jsonl(f1, [{"event": "iteration_start", "iteration": 1, "max_iterations": 75}])
        old_time = time.time() - 120
        os.utime(f1, (old_time, old_time))

        f2 = os.path.join(tmpdir, "agent_events_se_2_foo.jsonl")
        _write_jsonl(f2, [{"event": "iteration_start", "iteration": 5, "max_iterations": 75}])
...
def test_agent_state_is_done_all_four():
    from dashboard import AgentState
    state = AgentState(
        wt_path="/tmp/x", scenario_name="test",
...
def test_format_tool_call_write_file():
    from dashboard import format_tool_call
    assert format_tool_call("write_file", {"path": "tests/test_foo.py", "content": "..."}) == "w: tests/test_foo.py"
...
def test_format_tool_call_run_command():
    from dashboard import format_tool_call
    assert format_tool_call("run_command", {"command": "pytest tests/ -v"}) == "$ pytest tests/ -v"


# BDD: format_tool_call formats edit_file as "e: <path>"
def test_format_tool_call_edit_file():
    from dashboard import format_tool_call
    result = format_tool_call("edit_file", {"path": "src/lib.rs", "old_string": "x", "new_string": "y"})
...
def test_is_log_noise_thrown_away_line():
    from dashboard import is_log_noise
    assert is_log_noise("    → THROWN AWAY (no commits — agent made no progress)") is False
...
def test_parse_wt_mapping_line_extracts():
    ...
    result = parse_wt_mapping_line("  Enable verbose mode → /tmp/baadd-wt-enable-verbose-mode-5678")
...
def test_build_agent_panel_stale_title():
    from dashboard import AgentState, build_agent_panel
    state = AgentState(wt_path="/tmp/test", scenario_name="My Scenario", start_ts=time.time())
    with patch.object(type(state), 'is_stale', new_callable=PropertyMock, return_value=True):
        result = build_agent_panel(state)
...
def test_build_agent_panel_not_stale_title():
    from dashboard import AgentState, build_agent_panel
    state = AgentState(wt_path="/tmp/test", scenario_name="My Scenario", start_ts=time.time())
    with patch.object(type(state), 'is_stale', new_callable=PropertyMock, return_value=False):
        result = build_agent_panel(state)
...
def test_watcher_mode_no_subprocess():
    ...
    with patch('dashboard.Live', return_value=mock_live_instance), \
...
def test_watcher_does_not_exit_on_single_empty_poll():
    ...
    with patch('dashboard.Live', return_value=mock_live_instance), \
         ...
         patch('dashboard.read_wt_state', return_value=MagicMock(
             scenario_name="foo", done_phases=[], active_phase=None,
             current_iter=0, max_iter=125, tokens=0, last_tools=[],
             elapsed_s=0, is_stale=False, is_done=False
         )), \
         patch('time.sleep'):
        run_watcher_mode(0, Console())
...
def test_watcher_resolves_names_from_slug():
    from dashboard import slug_to_name, resolve_display_name
...
def test_render_progress_bar_exact_width():
    ...
    for current, max_v in [(0, 0), (0, 125), (62, 125), (125, 125), (200, 125)]:
        ...
        assert len(result) == 20, f"Expected len=20 for ({current}, {max_v}), got {len(result)}"
