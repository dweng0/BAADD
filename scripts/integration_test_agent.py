...
def run_tests(main_dir):
    ...
    if os.path.exists(bdd_config_path):
        _, _, rc = run_cmd(
            f'eval "$(python3 scripts/parse_bdd_config.py BDD.md)" && echo "$TEST_CMD"',
            cwd=main_dir,
            timeout=10,
        )
        if rc == 0:
...
def analyze_test_failure(stdout, stderr, rc):
    ...
    if "ImportError" in stdout or "ImportError" in stderr or "ModuleNotFoundError" in stdout or "ModuleNotFoundError" in stderr:
        ...
        if "import" in suggestion.lower():
            ...
            for file_line in files_out.splitlines():
                if "SyntaxError" in file_line:
                    ...
                    if m:
                        file_path = m.group(1)
                        log_event(log_path, "fix_attempt", file=file_path, fix="syntax_check")
...
def run_integration_tests(scenario_results, main_dir):
    ...
    if suggestions:
        ...
        if fixes_applied:
            ...
            if rc2 == 0:
                print(f"[OK] Tests now passing after fix ({passed2} passed, {failed2} failed)")
                return True, test_results
...
def main():
    ...
    parser.add_argument("--max-fix-attempts", type=int, default=1, help="Max fix attempts")
